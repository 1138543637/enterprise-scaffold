...
当前项目二“云网融合 AIOps 智能运维平台”已完成根因分析简化版能力。

已新增 aiops_root_cause_record 根因分析记录表。

已新增 POST /api/aiops/root-causes/analyze、
GET /api/aiops/root-causes/page、
GET /api/aiops/root-causes/{id} 接口。

已新增前端页面 /aiops/root-causes。

本阶段基于 A2-04 已完成的告警事件、指标数据、运维工单和资源台账进行简化规则分析，
可以生成疑似根因、分析证据、处理建议和置信度。

根因类型包括 CPU_HIGH、MEMORY_HIGH、DISK_FULL、NETWORK_ABNORMAL、
DATABASE_SLOW 和 UNKNOWN。

A2-05 继续复用 JWT、ApiResult、PageResult、MyBatis-Plus、@OperLog、
Vue3、Element Plus 和 Docker Compose。

本阶段不新增 Docker 服务，
但需要重新构建后端和前端镜像完成 Docker Compose 验收。
...
项目三 R3 银行实时交易风控与反欺诈平台已经开始接入 enterprise-scaffold。本阶段新增后端业务包 cn.sxu.enterprise.module.risk，新增 RiskHealthController，并提供 GET /api/risk/health 健康检查接口。该接口需要登录后携带 JWT 访问，返回 ApiResult 统一结构，并通过 @OperLog 记录操作日志。R3 后续固定使用 /api/risk/** 作为后端接口前缀，使用 risk_ 作为数据库表前缀，使用 /risk/** 作为前端路由前缀。本阶段不新增数据库表、不新增 SQL 文件、不新增 Docker 服务，Docker Compose 验收仍然必须执行。


### R3-02：银行交易模拟

R3-02 已新增银行交易模拟能力，新增 `risk_transaction` 交易流水表，新增 `RiskTransaction*` 后端分层代码，新增 `/api/risk/transactions/simulate`、`/api/risk/transactions/latest`、`/api/risk/transactions/page` 接口，新增 `/risk/transactions` 前端页面。本阶段继续复用 JWT、ApiResult、PageResult、@OperLog、MyBatis-Plus、Vue3、Element Plus 和 Docker Compose。
...
### I5-04:接口限流
I5-04：接口限流已完成。项目五“统一身份认证、权限审计与数据安全平台”已新增接口限流规则管理和模拟检测能力，新增 iam_rate_limit_rule 表，新增后端分页接口 GET /api/iam/rate-limit-rules/page，新增规则启用接口 POST /api/iam/rate-limit-rules/{id}/enable，新增规则停用接口 POST /api/iam/rate-limit-rules/{id}/disable，新增模拟检测接口 POST /api/iam/rate-limit-rules/simulate，新增前端 API 文件 scaffold-frontend/src/api/iam/rateLimitRule.ts，新增前端页面 scaffold-frontend/src/views/iam/IamRateLimitRuleView.vue，新增前端路由 /iam/rate-limit-rules。本阶段继续复用 JWT、ApiResult、PageResult、@OperLog 和 Docker Compose。本阶段不新增 Docker 服务、不修改 Docker 配置，不做真实 Filter / Interceptor 强制限流。
...
截至 G-01，Enterprise Scaffold 已形成一个完整的企业级多项目实战脚手架，包含公共企业级脚手架、项目一智能矿山平台、项目二云网融合 AIOps 智能运维平台、项目三银行实时交易风控与反欺诈平台、项目四国企 / 政务数据治理与共享交换平台、项目五统一身份认证、权限审计与数据安全平台。

当前系统最终能力包括：Spring Boot 3 后端基础架构、MySQL 数据库、MyBatis-Plus 分页与条件查询、JWT 登录认证、RBAC 基础权限查询、操作日志、登录日志、字典管理、文件上传、Vue3 + Vite + TypeScript 前端、Element Plus 后台页面、Axios 请求封装、Vue Router 路由管理、Pinia 状态管理、Docker Compose 一键部署、EMQX MQTT、Prometheus、Grafana、Kafka，以及五个完整业务项目的前后端闭环。

G-01 全项目验收范围包括：Dashboard 首页展示顺序验收、Docker Compose 一键启动验收、全项目页面冒烟测试、全项目接口冒烟测试、README 和 docs 总收尾、长期维护规则整理、简历项目描述整理、面试讲法整理、部署说明整理和 GitHub 提交收尾。

Dashboard 首页展示顺序固定为：公共脚手架 -> 项目一智能矿山 -> 项目二 AIOps -> 项目三银行风控 -> 项目四数据治理 -> 项目五 IAM 安全治理。

长期维护规则固定为：只修 bug，不随意改命名，不随意改变包名、表名、字段名、接口路径、前端路由、Docker 容器名、端口、环境变量、volume 和上传目录挂载规则；不随意引入新框架；如需新增项目六，必须重新规划新项目阶段。
