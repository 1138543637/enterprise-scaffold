...
G-01 是全项目总体验收与长期维护阶段，本阶段不新增后端接口，不修改既有接口路径，不修改 Controller、Service、Mapper、Entity、DTO、VO、Query、Request，不修改 ApiResult 和 PageResult 返回结构，不修改 JWT 认证机制，不修改 SecurityConfig 放行规则。

截至 G-01，系统已经形成公共脚手架接口和五个业务项目接口。公共接口覆盖登录认证、用户、角色、菜单、字典、文件上传、操作日志和登录日志。项目一接口使用 /api/mine/** 前缀，支撑智能矿山设备、传感器、数据、告警、工单、健康评分、预测性维护和看板。项目二接口使用 /api/aiops/** 前缀，支撑资源、指标、告警、工单、根因分析和运维看板。项目三接口使用 /api/risk/** 前缀，支撑银行交易、Kafka 交易接入、风控规则、风险识别、人工审核和风控看板。项目四接口使用 /api/datahub/** 前缀，支撑数据源、元数据、质量检测、敏感识别、脱敏预览、API 发布和数据治理看板。项目五接口使用 /api/iam/** 前缀，支撑 IAM 健康检查、接口访问日志、异常登录检测、接口限流规则、安全策略配置、权限审计、安全看板和风险闭环。

所有业务接口继续遵守 JWT 认证规则。分页接口继续使用 PageResult。普通接口继续使用 ApiResult。Controller 方法继续使用 @OperLog 记录操作日志。后续维护不得随意改变接口路径、请求参数、响应字段、分页结构和状态枚举。