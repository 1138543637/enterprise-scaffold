...
## I5-06：iam_permission_audit 权限审计表

I5-06 新增 SQL 文件 `scaffold-sql/i5_06_iam_permission_audit.sql`，SQL 第一行固定为 `SET NAMES utf8mb4;`，第二段固定使用 `USE enterprise_scaffold;`。本阶段新增表 `iam_permission_audit`，字段包括 `id`、`audit_code`、`audit_type`、`target_type`、`target_id`、`target_name`、`change_action`、`before_value`、`after_value`、`risk_level`、`review_status`、`review_by`、`review_time`、`request_ip`、`operator_name`、`status`、`create_by`、`create_time`、`update_by`、`update_time`、`deleted`、`remark`。其中 `audit_type` 表示审计类型，当前包括 `USER_ROLE`、`ROLE_MENU`、`USER_STATUS`、`MENU_PERMISSION`、`DATA_SCOPE`；`target_type` 表示对象类型，当前包括 `USER`、`ROLE`、`MENU`、`API`；`change_action` 表示变更动作，当前包括 `GRANT`、`REVOKE`、`UPDATE`、`ENABLE`、`DISABLE`；`risk_level` 表示风险等级，当前包括 `LOW`、`MEDIUM`、`HIGH`；`review_status` 表示复核状态，当前包括 `PENDING`、`REVIEWED`、`IGNORED`。该表继续使用 `iam_` 前缀，`deleted` 字段继续作为逻辑删除字段。


## I5-07：IAM 安全看板增强数据库说明

I5-07 不新增 SQL 文件，不新增数据库表，不新增字段，不修改已有表结构，因此不创建 `scaffold-sql/i5_07_xxx.sql`。本阶段安全看板只读取已有 IAM 表进行统计汇总：`iam_access_log` 用于统计今日访问量和今日失败访问量；`iam_login_risk` 用于统计异常登录风险、未处理风险和登录风险等级分布；`iam_rate_limit_rule` 用于统计限流规则启停状态；`iam_security_policy` 用于统计安全策略启停状态；`iam_permission_audit` 用于统计高风险权限变更、待复核审计、复核状态分布和最近权限审计事件。后续如果继续做趋势图或历史快照，再考虑新增 `iam_dashboard_snapshot` 一类汇总表；当前阶段不需要新增表。

## I5-08
项目五已完成 I5-08，新增 IAM 风险闭环处理能力。IAM 模块在 I5-07 安全看板增强基础上，继续围绕 iam_login_risk 和 iam_permission_audit 形成风险治理闭环。异常登录风险支持确认、忽略、关闭，并记录处理人、处理时间和处理备注；权限审计继续复用 reviewStatus、reviewBy、reviewTime 和 remark，支持复核和忽略。处理完成后，I5-07 安全看板中的未处理登录风险数和待复核审计数会随数据状态变化而下降，从而形成“发现风险 -> 展示风险 -> 人工处理 -> 留痕复核 -> 看板汇总”的完整链路。本阶段只做管理后台闭环处理，不做真实登录拦截，不做真实权限拦截，不重写 /api/auth/login，不重写 JWT，不修改 SecurityConfig。
...
I5-09 阶段不新增数据库表，不修改数据库字段，不新增 SQL 文件。
...
原因是项目五核心数据库表已经在 I5-01 至 I5-08 阶段完成，I5-09 只负责 Dashboard 首页集成、总体验收和文档收尾。
...
| 表名 | 职责 |
|---|---|
| `iam_access_log` | 记录接口访问日志，支撑接口访问留痕和安全审计 |
| `iam_login_risk` | 记录异常登录风险，支撑异常登录检测和风险闭环 |
| `iam_rate_limit_rule` | 存储接口限流规则，支撑重点接口访问频率控制 |
| `iam_security_policy` | 存储安全策略配置，支撑密码、登录、会话等策略管理 |
| `iam_permission_audit` | 记录权限变更审计，支撑权限复核和风险处理 |
...
I5-09 阶段数据库验收不看新增表，而是检查已有表能否支撑项目五页面和看板。

验收标准如下：

```text
1. iam_access_log 可以支撑接口访问日志分页查询；
2. iam_login_risk 可以支撑异常登录检测和风险闭环处理；
3. iam_rate_limit_rule 可以支撑限流规则管理；
4. iam_security_policy 可以支撑安全策略配置；
5. iam_permission_audit 可以支撑权限审计增强；
6. IAM 安全看板可以汇总以上数据；
7. 风险闭环处理后，未处理风险数量可以下降。
```
...
I5-09 阶段没有新增 SQL 文件，因此本阶段无需新增 SQL 初始化脚本。

************************************************************************************************************************************************************************

## G-01：全项目数据库最终状态说明

G-01 是全项目总体验收与长期维护阶段，本阶段不新增 SQL 文件，不新增数据库表，不新增数据库字段，不修改数据库结构，不新增索引，不修改初始化数据。

截至 G-01，数据库设计已经覆盖公共脚手架与五个业务项目。公共脚手架使用系统基础表支撑用户、角色、菜单、权限、字典、操作日志、登录日志和文件上传记录。项目一智能矿山使用 mine_ 前缀相关表支撑设备、传感器、传感器数据、告警、工单、健康评分、维护任务和看板统计。项目二 AIOps 使用 aiops_ 前缀相关表支撑资源、指标、告警、工单、根因分析和运维看板。项目三银行风控使用 risk_ 前缀相关表支撑交易流水、风控规则、规则命中、风险评分、人工审核和风控看板。项目四数据治理使用 datahub_ 前缀相关表支撑数据源、元数据、数据质量、敏感数据识别、脱敏规则、API 共享发布和调用日志。项目五 IAM 使用 iam_ 前缀相关表支撑接口访问日志、异常登录风险、接口限流规则、安全策略配置、权限审计和风险闭环。

G-01 不创建 scaffold-sql/g_01_xxx.sql。后续如果新增 SQL 文件，仍必须遵守固定规则：SQL 文件第一行必须是 SET NAMES utf8mb4;，第二段必须是 USE enterprise_scaffold;。后续维护数据库时，不得随意重命名已有表、字段、索引和业务状态值，避免破坏后端 Entity、Mapper、Service、VO、前端 API 和页面字段映射。
