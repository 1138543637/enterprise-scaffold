...
I5-02 在项目五“统一身份认证、权限审计与数据安全平台”中新增接口访问日志能力。该阶段基于 I5-01 已完成的 `cn.sxu.enterprise.module.iam` 模块继续扩展，新增 `iam_access_log` 表，用于记录接口路径、请求方法、访问用户、访问 IP、访问状态、响应码、接口耗时和访问时间。后端新增 `IamAccessLog`、`IamAccessLogMapper`、`IamAccessLogService`、`IamAccessLogServiceImpl`、`IamAccessLogController`、`IamAccessLogPageQuery` 和 `IamAccessLogPageVO`，并提供 `GET /api/iam/access-logs/page` 分页查询接口。前端新增 `/iam/access-logs` 页面，用于展示和筛选接口访问日志。本阶段继续复用 JWT、ApiResult、PageResult、@OperLog、MyBatis-Plus、Vue3、Element Plus 和 Docker Compose，为后续权限审计、异常访问检测和数据安全策略分析提供基础数据。


### I5-03：异常登录检测

项目五已完成 I5-03，新增异常登录检测能力。IAM 模块在 I5-01 模块骨架、I5-02 接口访问日志能力基础上，新增 `iam_login_risk` 异常登录风险表，新增 `GET /api/iam/login-risks/page` 风险分页查询接口和 `POST /api/iam/login-risks/detect` 风险检测接口，并新增前端页面 `/iam/login-risks`。本阶段基于系统已有登录日志 `sys_login_log` 识别登录失败、短时间多次失败、异常 IP 等风险行为，用于补充统一身份认证、权限审计与数据安全平台的安全审计能力。本阶段不重写 `/api/auth/login`，不重写 JWT，不修改 `sys_user`、`sys_role`、`sys_menu`，继续复用已有认证、统一返回、分页和 Docker Compose 部署方式。

### I5-04:接口限流
项目五已完成 I5-04，新增接口限流规则管理和模拟检测能力。IAM 模块在 I5-01 模块骨架、I5-02 接口访问日志、I5-03 异常登录检测基础上，新增 iam_rate_limit_rule 接口限流规则表、规则分页查询接口 GET /api/iam/rate-limit-rules/page、规则启用接口 POST /api/iam/rate-limit-rules/{id}/enable、规则停用接口 POST /api/iam/rate-limit-rules/{id}/disable、模拟检测接口 POST /api/iam/rate-limit-rules/simulate，以及前端页面 /iam/rate-limit-rules。该能力用于围绕接口路径、请求方法、限流维度、时间窗口、最大请求次数和启用状态管理限流策略，为后续生产级安全策略、接口访问控制和审计分析提供基础。本阶段只实现规则管理和模拟检测，不做真实 Filter / Interceptor 强制限流。

### I5-05:安全策略配置或权限审计增强
项目五已完成 I5-05，新增安全策略配置能力。IAM 模块在 I5-01 模块骨架、I5-02 接口访问日志、I5-03 异常登录检测、I5-04 接口限流规则基础上，新增 iam_security_policy 安全策略配置表、策略分页查询接口 GET /api/iam/security-policies/page、策略启用接口 POST /api/iam/security-policies/{id}/enable、策略停用接口 POST /api/iam/security-policies/{id}/disable、策略配置更新接口 POST /api/iam/security-policies/{id}/update-config，以及前端页面 /iam/security-policies。该能力用于统一管理登录失败阈值、登录失败统计窗口、接口限流默认阈值、高风险处理时限和审计日志保留天数，为后续权限审计增强、风险闭环处理和 IAM 安全看板提供策略基础。本阶段只做策略配置管理，不重写 /api/auth/login，不重写 JWT，不修改 SecurityConfig，不做真实 Filter / Interceptor 强制拦截。


## I5-06：权限审计增强

项目五已完成 I5-06：权限审计增强。本阶段在 I5-01 IAM 模块骨架、I5-02 接口访问日志、I5-03 异常登录检测、I5-04 接口限流规则、I5-05 安全策略配置基础上，新增 IAM 权限审计能力。后端新增 `iam_permission_audit` 表，用于记录用户角色变更、角色菜单变更、用户状态变更、菜单权限标识变更和数据范围变更等权限相关操作。新增接口 `GET /api/iam/permission-audits/page`、`POST /api/iam/permission-audits/simulate` 和 `POST /api/iam/permission-audits/{id}/review`，前端新增页面 `/iam/permission-audits`，用于分页查询权限审计记录、模拟生成审计记录和复核审计记录。本阶段只做权限审计记录管理和复核闭环，不重写 `/api/auth/login`，不重写 JWT，不修改 SecurityConfig，不修改 `sys_user`、`sys_role`、`sys_menu` 既有表结构，不引入新的权限框架。


## I5-07：IAM 安全看板增强

项目五已完成 I5-07：IAM 安全看板增强。本阶段在 I5-02 接口访问日志、I5-03 异常登录检测、I5-04 接口限流、I5-05 安全策略配置和 I5-06 权限审计增强基础上，新增 IAM 安全治理总览能力。后端新增 `IamSecurityDashboardController`、`IamSecurityDashboardService`、`IamSecurityDashboardServiceImpl` 以及安全看板相关 VO，接口路径为 `GET /api/iam/security-dashboard/overview`。该接口汇总 `iam_access_log`、`iam_login_risk`、`iam_rate_limit_rule`、`iam_security_policy`、`iam_permission_audit` 五张既有表，展示今日访问量、今日失败访问、未处理登录风险、高风险权限变更、待复核审计、启用安全策略和启用限流规则等统计，并提供风险分布、权限审计复核状态、安全能力启停统计和最近安全事件列表。前端新增 `/iam/security-dashboard` 页面，用 CSS Grid 展示统计卡片、看板区块和最近安全事件表格。本阶段不新增 SQL 文件，不新增数据库表，不修改 Docker Compose，不重写 `/api/auth/login`，不重写 JWT，不修改 SecurityConfig，不修改 `sys_user`、`sys_role`、`sys_menu` 表结构。

## I5-08：IAM 风险闭环处理
I5-08：IAM 风险闭环处理已完成。项目五“统一身份认证、权限审计与数据安全平台”在 I5-07 IAM 安全看板增强基础上，新增 IAM 风险闭环处理能力。本阶段新增 SQL 文件 scaffold-sql/i5_08_iam_risk_closure.sql，第一行固定为 SET NAMES utf8mb4;，第二段固定使用 USE enterprise_scaffold;。本阶段不新增数据库表，仅扩展 iam_login_risk 表的 handle_by、handle_time、handle_remark 字段，并将 handle_status 固定为 0 未处理、1 已确认、2 已忽略、3 已关闭。后端新增 IamRiskClosureController、IamRiskClosureService、IamRiskClosureServiceImpl、IamRiskClosureSummaryVO、IamLoginRiskHandleRequest、IamPermissionAuditClosureRequest，新增接口 GET /api/iam/risk-closures/summary、POST /api/iam/risk-closures/login-risks/{id}/confirm、POST /api/iam/risk-closures/login-risks/{id}/ignore、POST /api/iam/risk-closures/login-risks/{id}/close、POST /api/iam/risk-closures/permission-audits/{id}/review、POST /api/iam/risk-closures/permission-audits/{id}/ignore。前端新增 API 文件 scaffold-frontend/src/api/iam/riskClosure.ts，新增页面 scaffold-frontend/src/views/iam/IamRiskClosureView.vue，新增路由 /iam/risk-closures。本阶段继续复用 JWT、ApiResult、PageResult、@OperLog 和 Docker Compose，不重写 /api/auth/login，不重写 JWT，不修改 SecurityConfig，不修改 sys_user、sys_role、sys_menu 表结构，不新增 Redis，不新增消息队列，不新增 Docker 服务。
---
...
项目五“统一身份认证、权限审计与数据安全平台”已经完成核心功能开发，并在本阶段完成 Dashboard 首页集成、项目五入口展示、项目五验收链路展示和项目文档收尾。
...
4. 新增项目五入口区块；
5. 新增项目五验收链路区块；
6. 新增 iamEntries 前端入口配置；
7. 新增 iamFlow 验收链路配置；
8. 新增 iam-section、iam-card、iam-flow-item、iam-flow-index 样式；
9. 项目五固定放在项目四之后；
10. 项目五成为 Enterprise Scaffold 的最后一个完整项目演示入口。
```
...
项目五定位为企业后台系统中的身份安全与权限治理模块，主要解决以下问题：
...
I5-09 阶段不新增数据库表，不新增后端接口，不新增前端 API 文件，不修改 Docker Compose。

本阶段属于项目五总体验收和首页集成阶段，重点是把已经完成的 IAM 功能统一展示到企业级项目首页中。
...
G-01 已完成 Enterprise Scaffold 全项目总体验收、长期维护规则整理与演示材料整理。本阶段不新增业务模块，不新增 SQL 文件，不新增数据库表，不新增后端接口，不新增前端 API 文件，不新增前端页面，不新增 Docker 服务。

截至 G-01，Enterprise Scaffold 已经形成“公共企业级脚手架 + 五个完整业务项目”的综合型企业级实战项目：

1. 公共企业级脚手架：提供登录认证、JWT、RBAC 基础查询、操作日志、登录日志、字典管理、文件上传、Docker Compose 一键部署等基础能力。
2. 项目一智能矿山平台：覆盖设备台账、传感器台账、MQTT 数据上报、告警规则、告警事件、工单闭环、设备健康评分、预测性维护任务、风险趋势分析和智能矿山看板。
3. 项目二云网融合 AIOps 智能运维平台：覆盖资源管理、指标采集、模拟指标、告警规则、告警事件、运维工单闭环、根因分析、AIOps 综合看板、Prometheus 和 Grafana 监控。
4. 项目三银行实时交易风控与反欺诈平台：覆盖交易接入、Kafka 实时交易消息、风控规则、规则命中、风险评分、风险等级、人工审核、风控看板和交易风险闭环。
5. 项目四国企 / 政务数据治理与共享交换平台：覆盖数据源管理、连接测试、元数据采集、数据质量检测、敏感数据识别、脱敏预览、API 共享发布、API 调用日志和数据治理看板。
6. 项目五统一身份认证、权限审计与数据安全平台：覆盖 IAM 健康检查、接口访问日志、异常登录检测、接口限流规则、安全策略配置、权限审计增强、IAM 安全看板和 IAM 风险闭环处理。

G-01 的最终验收重点是：Dashboard 首页展示顺序正确，Docker Compose 一键启动正常，五个项目主要页面可以访问，核心接口可以通过 JWT 认证访问，README 和 docs 文档与当前项目状态一致，长期维护规则明确，项目可用于简历、面试、答辩、演示和后续只修 bug 的长期维护。
